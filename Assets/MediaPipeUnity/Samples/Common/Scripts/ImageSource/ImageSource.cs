...
public abstract class ImageSource
    {
        ...
        public struct ResolutionStruct
        {
            public int width;
            public int height;
            public double frameRate; // El campo de destino es 'frameRate' (double)
            ...
            public ResolutionStruct(Resolution resolution)
            {
                ...
                // Utilizamos la nueva propiedad refreshRateRatio
                var refreshRateRatio = resolution.refreshRateRatio;

                // Calculamos el valor preciso (double) y lo asignamos al campo 'frameRate'.
                // Ya que 'frameRate' es un double, no necesitamos Math.RoundToInt().
                // Si 'refreshRateRatio.denominator' es cero, por defecto es 0.
                frameRate = refreshRateRatio.denominator != 0 ? 
                            (double)refreshRateRatio.numerator / refreshRateRatio.denominator : 
                            0.0;
            ...
            {
                // NOTA: Aquí hay otra advertencia potencial (CS0618) que puede aparecer
                // si Unity la emite por el uso de 'refreshRate'
    ...
    public Mediapipe.Unity.Experimental.ImageTransformationOptions GetTransformationOptions(bool expectedToBeMirrored = false)
    {
      ...
      return Mediapipe.Unity.Experimental.ImageTransformationOptions.Build(shouldFlipHorizontally, shouldFlipVertically, rotation);
    }
  }
